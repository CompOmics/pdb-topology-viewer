...
class PdbTopologyViewerPlugin {
  ...
  menuStyle =
    'position:relative;height:38px;line-height:38px;background-color:#696969;padding: 0 10px;font-size:16px; color: #efefef;';
  ...
  async getApiData(pdbId: string, chainId: string) {
    ...
    return Promise.all(dataUrls.map((url) => fetch(url))).then((resp) =>
      Promise.all(
        resp.map((r) => {
          if (r.status == 200) {
            return r.json();
      ...
      if (topologyData[secStrType]) {
        ...
        topologyData[secStrType].forEach((secStrData: any) => {
          if (
            ...
            allCordinatesArray = allCordinatesArray.concat(
              this.chunkArray(secStrData.path, 2),
            );
          }
        });
      }
  ...
  drawStrandSubpaths(
    startResidueNumber: number,
    stopResidueNumber: number,
    index: number,
  ) {
    const _this = this;
    const totalAaInPath = stopResidueNumber - startResidueNumber + 1;
    const subPathHeight =
      (this.scaledPointsArr[7] - this.scaledPointsArr[1]) / totalAaInPath;
    ...
    this.svgEle
      ...
      .attr('class', (d: any, i: number) => {
        return (
          'strandsSubPath subpath-strands' +
          index +
          ' topo_res_' +
          d.residue_number
        );
  ...
  drawStrandMaskShape(index: number) {
    ...
    for (
      let maskPtIndex1 = 0;
      maskPtIndex1 < subtractIndexLength;
      maskPtIndex1++
    ) {
  ...
  renderTooltip(elementData: any, action: string, event?: any) {
    ...
    if (action === 'show') {
      ...
      if (typeof elementData.tooltipMsg !== 'undefined') {
        if (
          typeof elementData.tooltipPosition !== 'undefined' &&
  ...
  getResiduePath(residueNumber: number) {
    return this.svgEle.select('.topo_res_' + residueNumber);
  }

  isResidueSelected(residueEle: any) {
    return residueEle.classed('selectedResidue');
  }

  applyRestingResidueStyle(residueEle: any, residueData: any) {
  ...
  applySelectedResidueStyle(residueEle: any, residueData: any) {
  ...
  clearSelectedResidues(excludedResidueNumber?: number) {
    const _this = this;

    this.svgEle.selectAll('.selectedResidue').each(function (d: any) {
      if (
        typeof excludedResidueNumber !== 'undefined' &&
  ...
  mouseoverAction(event: any, eleObj: any | this, eleData: any) {
  ...
  mouseoutAction(event: any, eleObj: any, eleData: any) {
  ...
  drawHelicesSubpaths(
    ...
    let subPathHeight =
      (this.scaledPointsArr[9] - curveYdiff2 - this.scaledPointsArr[3]) /
      totalAaInPath;
    let startPoint = 0;
    if (curveYdiff === 0) {
      let boxHeight =
    ...
    this.svgEle
      ...
      .attr('class', function (d: any) {
        return (
          'helicesSubPath subpath-helices' +
          index +
          ' topo_res_' +
          d.residue_number
        );
  ...
  drawCoilsSubpaths(
      ...
      for (let subPathIndex = 0; subPathIndex < totalAaInPath; subPathIndex++) {
        ...
        if (cordArrPositon === 1) {
          ...
          if (prevCordArrPositon === undefined) {
            newSubPathCords['pathData'] = _this.scaledPointsArr.slice(
              0,
              cordArrPositon * 2,
            );
          } else {
            newSubPathCords['pathData'] = _this.scaledPointsArr.slice(
              prevCordArrPositon * 2,
              cordArrPositon * 2,
            );
    ...
    const termsData =
      this.apiData[2][this.entryId][this.entityId][this.chainId].terms;
    const totalCoilsInStr =
      this.apiData[2][this.entryId][this.entityId][this.chainId].coils.length;
    if (index === 0) {
      ...
      if (
        subPathCordsArr[totalAaInPath - 1]['pathData'][pathDataLen - 1] >
      ...
      this.svgEle
        ...
        .attr(
          'x',
          subPathCordsArr[totalAaInPath - 1]['pathData'][pathDataLen - 2],
        )
        .attr(
          'y',
          subPathCordsArr[totalAaInPath - 1]['pathData'][pathDataLen - 1] +
  ...
  drawTopologyStructures() {
    ...
    if (targetEleWt == 0 || this.targetEle.tagName === 'PDB-TOPOLOGY-VIEWER')
      targetEleWt = (this.targetEle.parentElement as HTMLElement).offsetWidth;
    if (targetEleHt == 0 || this.targetEle.tagName === 'PDB-TOPOLOGY-VIEWER')
      targetEleHt = (this.targetEle.parentElement as HTMLElement).offsetHeight;

    if (targetEleWt <= 330)
      (this.targetEle.querySelector('.menuDesc') as HTMLElement).innerText = `${
        this.entityId
      } | ${this.chainId.toUpperCase()}`;
  ...
  zoomDraw(event: any) {
    ...
    this.svgEle
      ...
      .attr('transform', function (d: any) {
        ...
        return (
          'translate(' + reszEleCordinates.x + ',' + reszEleCordinates.y + ')'
        );
      })
      .attr(
        'd',
    ...
    this.svgEle.selectAll('.residueSelection').attr('d', function (d: any) {
    ...
    this.svgEle
      ...
      .forEach((node: any) => this.svgEle.node().append(node));
  ...
  highlight(
    ...
    let fill: any = 'none';
    let fillOpacity = 0;
    let stroke: any = this.defaultColours.mouseOver;
    let strokeWidth = 1.2;
    let strokeOpacity = 1;
    ...
    for (
      let residueNumber = startResidue;
      residueNumber <= endResidue;
      residueNumber++
    ) {
        ...
        if (Object.entries(mappingsData[categoryArr[catIndex]]).length !== 0) {
          ...
          for (let accKey in mappingRecords) {
            mappingRecords[accKey].mappings.forEach((domainMappings: any) => {
              if (
                domainMappings.entity_id == this.entityId &&
  ...
  getChainStartAndEnd() {
    ...
    for (let chainIndex = 0; chainIndex < totalChainsInArr; chainIndex++) {
      if (chainsData[chainIndex].chain_id == this.chainId) {
        ...
        chainsData[chainIndex].observed.forEach(
          (observedData: any, observedDataIndex: number) => {
            ...
            if (chainDataObj.chain_id == this.chainId) {
              ...
              chainDataObj.models.forEach((chainModelObj: any) => {
                ...
                chainModelObj.residues.forEach((outlierResidue: any) => {
                  ...
                  if (
                    outlierResidue.outlier_types.length === 1 &&
                    outlierResidue.outlier_types[0] === 'RSRZ'
                  ) {
                    resColor = _this.defaultColours.qualityRed;
                    _this.drawValidationShape(
                      outlierResidue.residue_number,
                      'circle',
                      resColor,
                    );
                    ...
                    const otherOutlierIndex = otherOutliersTempArray.indexOf(
                      outlierResidue.residue_number,
                    );
                    if (otherOutlierIndex > -1) {
                      residueDetails[otherOutlierIndex]['tooltipMsg'] =
                        residueDetails[otherOutlierIndex]['tooltipMsg'].replace(
                          '<br>',
                          ', RSRZ<br>',
                        );
                    } else {
                  ...
                  let tooltipMsgText =
                    ...
                    '<br>';
                  const rsrzTempArrayIndex = rsrzTempArray.indexOf(
                    outlierResidue.residue_number,
                  );
  ...
  resetTheme() {
    ...
    this.svgEle.selectAll('.coloured').each(function (d: any) {
      ...
      if (
        nodeClassArr.indexOf('strandsSubPath') > -1 ||
  ...
  updateTheme(residueDetails: any) {
    ...
    residueDetails.forEach((residueDetailsObj: any) => {
      for (let i = residueDetailsObj.start; i <= residueDetailsObj.end; i++) {
        _this.changeResidueColor(
          i,
          residueDetailsObj.color,
          residueDetailsObj.tooltipMsg,
          residueDetailsObj.tooltipPosition,
        );
      }
    });
  }
  ...
  handleSeqViewerEvents(e: any, eType: string) {
    if (typeof e.eventData !== 'undefined') {
      ...
      if (
        e.eventData.entryId.toLowerCase() != this.entryId.toLowerCase() ||
      ...
      if (e.eventData.residueNumber) {
        ...
        e.eventData.elementData.pathData.start.residue_number &&
  ...
  handleMolstarEvents(e: any, eType: string) {
    if (
      ...
      if (
        e.eventData.entry_id.toLowerCase() != this.entryId.toLowerCase() ||
