...
class PdbTopologyViewerPlugin {
    constructor() {
        ...
        this.resizeDebounceMs = 150; // tweak if needed
                ...
                if (typeof mappingsData[categoryArr[catIndex]] !== 'undefined') {
                    if (Object.entries(mappingsData[categoryArr[catIndex]]).length !== 0) {
                        ...
                        for (let accKey in mappingRecords) {
                            mappingRecords[accKey].mappings.forEach((domainMappings) => {
                                if (domainMappings.entity_id == this.entityId &&
                                    domainMappings.chain_id == this.chainId) {
                                    residueDetails.push({
        ...
        this.createDomainDropdown = function () {
            ...
            if (this.domainTypes.length > 1) {
                let optionList = '';
                this.domainTypes.forEach((opt, i) => {
    ...
    render(target, options, apiData) {
        if (options &&
            typeof options.displayStyle != 'undefined' &&
            options.displayStyle != null)
            this.displayStyle += options.displayStyle;
        if (options &&
            typeof options.errorStyle != 'undefined' &&
            options.errorStyle != null)
            this.errorStyle += options.errorStyle;
        if (options &&
            typeof options.menuStyle != 'undefined' &&
            options.menuStyle != null)
        ...
        if (typeof options.chainId == 'undefined' || options.chainId == null) {
            this.getObservedResidues(this.entryId).then((result) => {
                if (typeof result != 'undefined' &&
                    typeof result[this.entryId] != 'undefined' &&
    ...
    resize() {
        ...
        const svgNode = this.svgEle.node();
        if (!svgNode)
            return;
        const currentTransform = d3.zoomTransform(svgNode);
        this.svgEle.transition().duration(0).call(this.zoom.transform, currentTransform);
        if (this.domainTypes && this.domainTypes[selectedIndex]) {
            const newSelectBox = this.targetEle.querySelector('.menuSelectbox');
            if (newSelectBox)
                newSelectBox.selectedIndex = selectedIndex;
            this.displayDomain();
        }
    }
    async initPainting(apiData) {
        const data = apiData !== null && apiData !== void 0 ? apiData : (await this.getApiData(this.entryId, this.chainId));
        if (!data)
            return;
    ...
    async getObservedResidues(pdbId) {
        try {
            return await (await fetch(`https://www.ebi.ac.uk/pdbe/api/pdb/entry/observed_residues_ratio/${pdbId}`)).json();
        }
        catch (e) {
            console.error(`Couldn't load UniProt variants`, e);
        }
    }
    async getApiData(pdbId, chainId) {
        const dataUrls = [
        ...
        return Promise.all(dataUrls.map((url) => fetch(url))).then((resp) => Promise.all(resp.map((r) => {
            if (r.status == 200) {
                return r.json();
            }
            else {
    ...
    getDomainRange() {
        ...
        for (let secStrType in topologyData) {
            if (topologyData[secStrType]) {
                ...
                topologyData[secStrType].forEach((secStrData) => {
                    if (typeof secStrData.path !== 'undefined' &&
        ...
        this.xScale = d3
            .scaleLinear()
            .domain([
            d3.min(allCordinatesArray, function (d) {
                return d[0];
            }),
            d3.max(allCordinatesArray, function (d) {
                return d[0];
            }),
        ])
        ...
        this.zoom = d3.zoom().on('zoom', (event) => this.zoomDraw(event));
        //.scaleExtent([.5, 20])  // This control how much you can unzoom (x0.5) and zoom (x20)
        // .transform(this.xScale, this.yScale)
    }
    drawStrandSubpaths(startResidueNumber, stopResidueNumber, index) {
        const _this = this;
        ...
        for (let subPathIndex = 0; subPathIndex < totalAaInPath; subPathIndex++) {
            let subPathObj = { type: 'strands', elementIndex: index };
            if (subPathIndex === 0) {
                ...
                subPathObj['pathData'] = [
                    ...
                    this.scaledPointsArr[4],
                    this.scaledPointsArr[1] + subPathHeight,
                ...
                subPathObj['pathData'] = [
                    dValArr[subPathIndex - 1]['pathData'][2],
                    dValArr[subPathIndex - 1]['pathData'][3],
                    dValArr[subPathIndex - 1]['pathData'][2],
                    dValArr[subPathIndex - 1]['pathData'][3] + subPathHeight,
                    dValArr[subPathIndex - 1]['pathData'][4],
                    dValArr[subPathIndex - 1]['pathData'][5] + subPathHeight,
                    dValArr[subPathIndex - 1]['pathData'][4],
                    dValArr[subPathIndex - 1]['pathData'][5],
                ];
            }
        ...
        this.svgEle
            .selectAll('.subpath-strands' + index)
            .data(dValArr)
            .enter()
            .append('path')
            .attr('class', (d, i) => {
            return ('strandsSubPath subpath-strands' +
                index +
                ' topo_res_' +
                d.residue_number);
        })
            .attr('d', (d, i) => {
            return 'M ' + d.pathData.join(' ') + ' Z';
        })
            .attr('stroke', '#111')
            .attr('stroke-width', '0')
            .attr('fill', 'white')
            ...
            _this.mouseoverAction(event, this, d);
        })
            .on('mousemove', function (event, d) {
            _this.mouseoverAction(event, this, d);
        })
            .on('mouseout', function (event, d) {
            _this.mouseoutAction(event, this, d);
        })
            .on('click', function (event, d) {
            _this.clickAction(d);
        });
    }
    drawStrandMaskShape(index) {
        ...
        for (let maskPtIndex = 0; maskPtIndex < addIndexLength; maskPtIndex++) {
            maskPointsArr[adjustIndexAddArr[maskPtIndex]] =
                maskPointsArr[adjustIndexAddArr[maskPtIndex]] + adjustmentFactor;
        }
        ...
        for (let maskPtIndex1 = 0; maskPtIndex1 < subtractIndexLength; maskPtIndex1++) {
            maskPointsArr[adjustIndexSubtractArr[maskPtIndex1]] =
                maskPointsArr[adjustIndexSubtractArr[maskPtIndex1]] - adjustmentFactor;
        }
        ...
        this.svgEle
            .selectAll('.maskpath-strands' + index)
            .data([maskPointsArr])
            .enter()
            .append('path')
            .attr('class', (d, i) => {
            return 'strandMaskPath maskpath-strands' + index;
        })
            .attr('d', (d, i) => {
            return 'M' + maskPointsArr.join(' ') + 'Z';
        })
            .attr('stroke', '#111')
    ...
    renderTooltip(elementData, action, event) {
        let toolTipEle = d3.select('.pdbTopologyTooltip');
        if (toolTipEle._groups[0][0] == null) {
            toolTipEle = d3
                .select('body')
                .append('div')
                .attr('class', 'pdbTopologyTooltip')
                .attr('style', 'display: none;width: auto;position: absolute;background: #fff;padding: 5px;border: 1px solid #666;border-radius: 5px;box-shadow: 5px 6px 5px 0 rgba(0,0,0,.17);font-size: .9em;color: #555;z-index: 998;');
        }
        if (action === 'show') {
            const x = event.pageX, y = event.pageY;
            let tooltipContent = 'Residue ' +
                elementData.residue_number +
                ' (' +
                this.sequenceArr[elementData.residue_number - 1] +
                ')';
            if (typeof elementData.tooltipMsg !== 'undefined') {
                if (typeof elementData.tooltipPosition !== 'undefined' &&
                    elementData.tooltipPosition === 'postfix') {
    ...
    dispatchEvent(eventType, eventData, eventElement) {
    ...
    mouseoverAction(event, eleObj, eleData) {
    ...
    drawHelicesSubpaths(startResidueNumber, stopResidueNumber, index, curveYdiff) {
        const _this = this;
        curveYdiff = 0;
        const diffVal = 5;
        ...
        if (curveYdiff === 0)
            curveYdiff2 = 0;
        let subPathHeight = (this.scaledPointsArr[9] - curveYdiff2 - this.scaledPointsArr[3]) /
            totalAaInPath;
        let startPoint = 0;
        if (curveYdiff === 0) {
            let boxHeight = this.svgEle
                .select('.helices' + index)
                .node()
                .getBBox().height +
                subPathHeight / 2;
        ...
        if (curveYdiff === 0) {
            for (let subPathIndex = 0; subPathIndex < totalAaInPath; subPathIndex++) {
                ...
                if (subPathIndex === 0) {
                    ...
                    subPathObj['pathData'] = [
                        ...
                        this.scaledPointsArr[3] + startPoint,
                        this.scaledPointsArr[4],
                        this.scaledPointsArr[3] + startPoint,
                        this.scaledPointsArr[4],
                        this.scaledPointsArr[3] + startPoint + subPathHeight,
                        this.scaledPointsArr[0],
                        this.scaledPointsArr[3] + startPoint + subPathHeight,
                    ];
                }
                    ...
                    subPathObj['pathData'] = [
                        dValArr2[subPathIndex - 1]['pathData'][6],
                        dValArr2[subPathIndex - 1]['pathData'][7],
                        dValArr2[subPathIndex - 1]['pathData'][4],
                        dValArr2[subPathIndex - 1]['pathData'][5],
                        dValArr2[subPathIndex - 1]['pathData'][4],
                        dValArr2[subPathIndex - 1]['pathData'][5] + subPathHeight,
                        dValArr2[subPathIndex - 1]['pathData'][6],
                        dValArr2[subPathIndex - 1]['pathData'][5] + subPathHeight,
                    ];
                }
            ...
            for (let subPathIndex = 0; subPathIndex < totalAaInPath; subPathIndex++) {
                ...
                if (subPathIndex === 0) {
                    subPathObj['residue_number'] = startResidueNumber;
                    subPathObj['pathData'] = [
                        _this.scaledPointsArr[0],
                        _this.scaledPointsArr[3] + curveYdiff2 / 2,
                        _this.scaledPointsArr[4],
                        _this.scaledPointsArr[3] + curveYdiff2 / 2,
                        _this.scaledPointsArr[4],
                        _this.scaledPointsArr[3] + subPathHeight + curveYdiff2 / 2,
                        _this.scaledPointsArr[0],
                        _this.scaledPointsArr[3] + subPathHeight + curveYdiff2 / 2,
                    ];
                }
        ...
        this.svgEle
            .selectAll('.subpath-helices' + index)
            .data(dValArr2)
            .enter()
            .append('path')
            .attr('class', function (d) {
            return ('helicesSubPath subpath-helices' +
                index +
                ' topo_res_' +
                d.residue_number);
        })
    ...
    drawHelicesMaskShape(index) {
        const adjustmentFactor = 0.3;
        let helicesMaskArr = [
            [
                this.scaledPointsArr[0] - adjustmentFactor,
                this.scaledPointsArr[1],
                this.scaledPointsArr[2],
                this.scaledPointsArr[3] - adjustmentFactor,
                this.scaledPointsArr[4] + adjustmentFactor,
            ...
            [
                this.scaledPointsArr[6] + adjustmentFactor,
                this.scaledPointsArr[7],
                this.scaledPointsArr[8],
                this.scaledPointsArr[9] + adjustmentFactor,
                this.scaledPointsArr[10] - adjustmentFactor,
        ...
        this.svgEle
            .selectAll('.maskpath-helices' + index)
            .data(helicesMaskArr)
            .enter()
            .append('path')
            .attr('class', function (d) {
            return 'helicesMaskPath maskpath-helices' + index;
        })
            .attr('d', function (d) {
            return ('M' +
                d[0] +
    ...
    drawCoilsSubpaths(startResidueNumber, stopResidueNumber, index) {
        const _this = this;
        ...
        if (totalAaInPath === 1) {
            ...
            for (let subPathIndex = 0; subPathIndex < totalAaInPath; subPathIndex++) {
                ...
                newSubPathCords['pathData'] = newSubPathCords['pathData'].concat([
                    subPathCord.x,
                    subPathCord.y,
                ]);
                subPathCordsArr.push(newSubPathCords);
        ...
        if (startResidueNumber !== -1 && stopResidueNumber !== -1) {
            ...
            this.svgEle
                .selectAll('.subpath-coils' + index)
                .data(subPathCordsArr)
                .enter()
                .append('path')
                .attr('class', function (d) {
                return ('coilsSubPath subpath-coils' +
                    index +
                    ' topo_res_' +
                    d.residue_number);
            })
                .attr('d', function (d) {
                return 'M ' + d.pathData.join(' ');
            })
                .attr('stroke', this.defaultColours.borderColor)
                .attr('stroke-width', 0.3)
                .attr('fill', 'none')
        ...
        if (index === 0) {
            ...
            if (subPathCordsArr[totalAaInPath - 1]['pathData'][pathDataLen - 1] >
                subPathCordsArr[totalAaInPath - 1]['pathData'][pathDataLen - 3]) {
                adjustmentFactor = 2;
            }
            ...
            this.svgEle
                ...
                .attr('y', subPathCordsArr[totalAaInPath - 1]['pathData'][pathDataLen - 1] +
                adjustmentFactor)
                .attr('stroke', '#ff0000')
                .attr('stroke-width', '0.3')
                // .attr('font-size', 3 * this.zoom.scale() +'px')
    ...
    drawTopologyStructures() {
        ...
        this.targetEle.innerHTML = `<div style="${this.displayStyle}">
            ...
            <div class="menuContainer" style="${this.menuStyle}">
                ...
                <a style="color: #efefef;border-bottom:none; cursor:pointer;margin-left: 20px;" target="_blank" href="https://pdbe.org/${this.entryId}">${this.entryId}</a> | <span class="menuDesc">Entity ${this.entityId} | Chain ${this.chainId.toUpperCase()}</span>
        ...
        const svgSection = this.targetEle.querySelector('.svgSection');
      ...
      <svg class="topoSvg"
        ...
        this.svgEle.call(this.zoom).on('contextmenu', function (event) {
            ...
            for (let secStrDataIndex = 0; secStrDataIndex < secStrArr.length; secStrDataIndex++) {
                ...
                if (typeof secStrData.path !== 'undefined' &&
                    ...
                    if (secStrType === 'terms') {
                        ...
                        const newEle = this.svgEle
                            .selectAll('path.' + secStrType + '' + secStrDataIndex)
                            .data([secStrData])
                            .enter()
                            .append('path')
                            .attr('class', () => {
                            if (secStrData.start === -1 &&
                                secStrData.stop === -1 &&
                                secStrType !== 'terms') {
                                return ('dashedEle topologyEle ' +
                                    secStrType +
                                    ' ' +
                                    secStrType +
                                    '' +
                            ...
                            for (let i = 0; i < pathLenth; i++) {
                                ...
                                if ((secStrType === 'helices' && i === 6) ||
                                    (secStrType === 'coils' &&
                                        secStrData.path.length < 12 &&
                                        i === 8))
                                    dVal += ' L';
                                if (xScaleFlag) {
                        ...
                        if (secStrType === 'strands') {
                            ...
                            this.svgEle.node().append(newEle.node());
                        }
                        //for helices
                        if (secStrType === 'helices') {
                            //create subsections/paths
        ...
        this.svgEle
            .node()
            .append(this.svgEle.selectAll('.validationResidue').node());
    }
    zoomDraw(event) {
        if (!this.xScale || !this.yScale) {
            console.warn('Scales not initialized before zoom');
            return;
        }
        const new_xScale = event.transform.rescaleX(this.xScale);
        const new_yScale = event.transform.rescaleY(this.yScale);
        // return
        const _this = this;
        ...
        pathEle.each(function (d) {
            d3.select(d3.select(this).node()).attr('d', function (d) {
                ...
                for (let i = 0; i < pathLenth; i++) {
                    ...
                    if ((d.secStrType === 'helices' && i === 6) ||
                        (d.secStrType === 'coils' && d.path.length < 12 && i === 8))
                        dVal += ' L';
                    if (xScaleFlag) {
            ...
            if (d.secStrType === 'helices') {
                ...
                //bring original/complete helices in front newEle
                _this.svgEle.node().append(d3.select(this).node());
            }
        ...
        this.svgEle
            ...
            if (residueEleData[0].type === 'strands' ||
                ...
                const coilCenter = residueEle
                    .node()
                    .getPointAtLength(residueEle.node().getTotalLength() / 2);
                reszEleCordinates = {
                    x: coilCenter.x,
                    y: coilCenter.y,
                };
        ...
        this.svgEle.selectAll('.residueSelection').attr('d', function (d) {
            //assign the d attribute of the corresponding sub-path
            const dataObj = d3.select(this).data();
            return _this.svgEle
                .select('.topo_res_' + dataObj[0].residueNumber)
                .attr('d');
        });
        ...
        this.svgEle
            ...
            .forEach((node) => this.svgEle.node().append(node));
    ...
    highlight(startResidue, endResidue, color, eventType) {
        ...
        for (let residueNumber = startResidue; residueNumber <= endResidue; residueNumber++) {
            ...
            this.svgEle
                ...
                _this.mouseoverAction(event, residueEleNode, residueEleData[0]);
            })
                .on('mousemove', (event, d) => {
                _this.mouseoverAction(event, residueEleNode, residueEleData[0]);
            })
                .on('mouseout', (event, d) => {
                _this.mouseoutAction(event, residueEleNode, residueEleData[0]);
            })
                .on('click', (event, d) => {
                _this.clickAction(residueEleData[0]);
            });
        }
    }
    drawValidationShape(residueNumber, shape, rgbColor) {
        const _this = this;
        //calculate Shape dimesions
        const residueEle = _this.svgEle.select('.topo_res_' + residueNumber);
    ...
    getChainStartAndEnd() {
        ...
        for (let chainIndex = 0; chainIndex < totalChainsInArr; chainIndex++) {
            if (chainsData[chainIndex].chain_id == this.chainId) {
                ...
                chainsData[chainIndex].observed.forEach((observedData, observedDataIndex) => {
    ...
    getAnnotationFromOutliers() {
        const _this = this;
        const chainRange = this.getChainStartAndEnd();
        let residueDetails = [
            {
        ...
        if (typeof outlierData !== 'undefined' &&
            typeof outlierData.molecules !== 'undefined' &&
            outlierData.molecules.length > 0) {
            outlierData.molecules.forEach((qualityData) => {
                if (qualityData.entity_id == this.entityId) {
                    //Iterate chains array in outliers
                    qualityData.chains.forEach((chainDataObj) => {
                        if (chainDataObj.chain_id == this.chainId) {
                            //Iterate models array in chains array in outliers
                            chainDataObj.models.forEach((chainModelObj) => {
                                //Iterate residues array in models array in outliers
                                chainModelObj.residues.forEach((outlierResidue) => {
                                    let resColor = _this.defaultColours.qualityYellow;
                                    let issueSpell = 'issue';
                                    if (outlierResidue.outlier_types.length === 1 &&
                                        outlierResidue.outlier_types[0] === 'RSRZ') {
                                        ...
                                        if (otherOutlierIndex > -1) {
                                            residueDetails[otherOutlierIndex]['tooltipMsg'] =
                                                residueDetails[otherOutlierIndex]['tooltipMsg'].replace('<br>', ', RSRZ<br>');
                                        }
                                        else {
                                    ...
                                    let tooltipMsgText = 'Validation ' +
                                        issueSpell +
                                        ': ' +
                                        outlierResidue.outlier_types.join(', ') +
                                        '<br>';
    ...
    resetTheme() {
        ...
        this.svgEle.selectAll('.coloured').each(function (d) {
            ...
            const nodeEle = d3.select(node).classed('coloured', false);
            //Change fill and border
            const nodeClassArr = nodeEle.attr('class').split(' ');
            if (nodeClassArr.indexOf('strandsSubPath') > -1 ||
                nodeClassArr.indexOf('helicesSubPath') > -1) {
    ...
    changeResidueColor(residueNumber, rgbColor, tooltipContent, tooltipPosition) {
    ...
    updateTheme(residueDetails) {
        const _this = this;
        residueDetails.forEach((residueDetailsObj) => {
    ...
    handleSeqViewerEvents(e, eType) {
        if (typeof e.eventData !== 'undefined') {
            //Abort if entryid and entityid do not match
            if (e.eventData.entryId.toLowerCase() != this.entryId.toLowerCase() ||
                e.eventData.entityId != this.entityId)
                return;
            //Abort if chain id is different
            if (e.eventData.elementData.pathData.chain_id &&
                e.eventData.elementData.pathData.chain_id != this.chainId)
                return;
            ...
            else if (e.eventData.elementData.pathData.start.residue_number &&
                e.eventData.elementData.pathData.end.residue_number) {
            ...
            if (typeof startResidue !== 'undefined' &&
                typeof endResidue !== 'undefined') {
                let color;
                if (e.eventData.elementData.color &&
                    e.eventData.elementData.color.length == 1) {
                    color = e.eventData.elementData.color[0];
                }
                else {
                    color = {
                        r: e.eventData.elementData.color[0],
                        g: e.eventData.elementData.color[1],
                        b: e.eventData.elementData.color[2],
                    };
                }
        ...
        if (typeof e.detail !== 'undefined') {
            ...
            if (typeof e.detail.feature != 'undefined') {
                if (typeof e.detail.feature.accession != 'undefined') {
                    ...
                    if (accessionArr[0] == 'Chain' &&
                        accessionArr[1].toLowerCase() != this.chainId.toLowerCase())
                        return;
                }
                if (e.detail.trackIndex > -1 &&
                    e.detail.feature.locations &&
                    e.detail.feature.locations[0].fragments[e.detail.trackIndex].color)
                    selColor =
                        e.detail.feature.locations[0].fragments[e.detail.trackIndex].color;
    ...
    handleMolstarEvents(e, eType) {
        if (typeof e.eventData !== 'undefined' &&
            Object.keys(e.eventData).length > 0) {
            //Remove previous selection / highlight
            let selectionPathClass = 'residueSelection';
            if (eType == 'mouseover') {
                selectionPathClass = 'residueHighlight';
            ...
            if (e.eventData.entry_id.toLowerCase() != this.entryId.toLowerCase() ||
                e.eventData.entity_id != this.entityId)
                return;
            //Abort if chain id is different
    ...
    subscribeWcEvents() {
        ...
        document.addEventListener('PDB.seqViewer.click', (e) => {
            this.handleSeqViewerEvents(e, 'click');
        });
        document.addEventListener('PDB.seqViewer.mouseover', (e) => {
        ...
        document.addEventListener('PDB.litemol.click', (e) => {
            ...
            if (e.eventData.entryId.toLowerCase() != this.entryId.toLowerCase() ||
                ...
                return;
            //Abort if chain id is different
            if (e.eventData.chainId.toLowerCase() != this.chainId.toLowerCase())
                return;
        ...
        document.addEventListener('protvista-mouseover', (e) => {
...
export default PdbTopologyViewerPlugin;
